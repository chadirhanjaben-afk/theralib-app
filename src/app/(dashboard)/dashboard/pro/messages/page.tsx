      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-brand-blue-gray text-sm mt-0.5">
            {totalUnread > 0 ? `${totalUnread} message${totalUnread > 1 ? 's' : ''} non lu${totalUnread > 1 ? 's' : ''}` : 'Communiquez avec vos clients'}
          </p>
        </div>
      </div>
          ...
          <div className="p-4 border-b border-gray-100">
            <div className="relative">
              <input
                ...
                className="w-full pl-9 pr-4 py-2.5 bg-gray-50 rounded-xl text-sm border-0 focus:ring-2 focus:ring-brand-teal/30 focus:bg-white transition-colors"
              />
          ...
          <div className="flex-1 overflow-y-auto">
            {conversations.length === 0 ? (
              <div className="text-center py-12 px-6">
                ...
                <p className="text-xs text-gray-400 mt-1">Les messages de vos clients apparaîtront ici</p>
              </div>
            ) : (
              conversations.map((conv) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left px-4 py-3.5 flex items-start gap-3 transition-colors border-b border-gray-50 ${
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center justify-between mt-0.5">
                        ...
                        {unread > 0 && (
                          <span className="ml-2 shrink-0 w-5 h-5 rounded-full bg-brand-teal text-white text-[10px] flex items-center justify-center font-bold">
                            {unread}
                          </span>
                        )}
                      </div>
          ...
          {!selectedConvId ? (
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-sm text-gray-400 mt-1">Choisissez une conversation dans la liste pour commencer</p>
              </div>
            </div>
          ) : (
            <>
              ...
              <div className="px-6 py-4 border-b border-gray-100 flex items-center gap-3">
                <button
                  className="md:hidden mr-2 text-gray-400 hover:text-gray-600"
                ...
                <div>
                  <p className="font-semibold text-sm">{selectedConv?.otherUser?.displayName || 'Utilisateur'}</p>
              ...
              <div className="px-6 py-4 border-t border-gray-100">
                <div className="flex items-center gap-3">
                  <input
                    ...
                    className="flex-1 px-4 py-3 bg-gray-50 rounded-xl text-sm border-0 focus:ring-2 focus:ring-brand-teal/30 focus:bg-white transition-colors"
                    disabled={sending}
                  />
                  <button
                    onClick={handleSend}
